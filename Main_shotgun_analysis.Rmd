...
#head(data.frame(sample_sums(final_physeq)))

# #noise filter
# #subset phyloseq object
# physeq_sub<-subset_samples(physeq, sample_sums(physeq) > 10000)
# physeq_sub

#subset samples
ino<-subset_samples(final_physeq, type == 'Ino-Mix')
ino
summarize_phyloseq(ino)
...
CairoSVG(file = "fig2a", width = 2, height = 3, bg = "white")
plot_composition(pseq_ino,
  ...
  labs(x = "Samples", y = "Relative abundance",
       title = "InoMix: Relative abundance data") + 
  theme_ipsum(grid="Y") +
  theme(legend.text = element_text(face = "italic")) + 
  guides(fill=guide_legend(title="Genus")) + th
...
plot_composition(pseq_mix1,
  ...
  guides(fill = guide_legend(ncol = 1)) +
  labs(x = "Samples", y = "Relative abundance",
       title = "Mix1: Relative abundance data") + 
  theme_ipsum(grid="Y") +
...
for (n in treatments){ # for each variable n in compounds
    ...
    ps.sub <- subset_samples(pseq_mix1, treatments == n) # Choose sample from compound by n
    
    #samples<-(sample_data(ps.sub))$sample
    #samples
    
    #for (i in sample){
    
    #samp<-subset_samples(ps.sub, sample = samples[i])
    core_m <- core_members(ps.sub, detection = 0.0001, include.lowest = TRUE) # ps.sub is phyloseq selected with only samples from g 
                           #)
    print(paste0("No. of core taxa in Mix-1 ", n," : ", length(core_m))) # print core taxa identified in each treatment.
...
#Mix1
df %>% filter(type=='Mix1') %>% ggplot(aes(x=passage,y=diversity_shannon)) + geom_point(aes(color=compound)) + geom_smooth(aes(fill=compound,color=compound), alpha=0.1) +
  scale_color_npg() + theme_bw() + scale_fill_npg() +
labs(x="Passage", y="Shannon diversity") + th + ggtitle("Alpha diversity over passages: Mix1")

#Mix2
df %>% filter(type=='Mix2' & compound != 'mGAM only') %>% ggplot(aes(x=passage,y=diversity_shannon)) + geom_point(aes(color=compound)) + geom_smooth(aes(fill=compound,color=compound), alpha=0.1) + scale_color_npg() + scale_fill_npg() + theme_bw() + 
...
#CairoSVG(file = 'Fig3', width = 8, height = 6, bg = 'white')
pwpp(em$emmeans) + scale_color_simpsons() + theme_minimal() + ggtitle("Mix1: pairwise comparison p-value plot")
#dev.off()
...
#pairwise p-value plot (uses 'Tukey' adjusted p-value)
#CairoSVG(file = 'Fig3', width = 8, height = 6, bg = 'white')
pwpp(em$emmeans) + scale_color_simpsons() + theme_minimal() + ggtitle("Mix2: pairwise comparison p-value plot")
#dev.off()
