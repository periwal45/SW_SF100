...
#' CairoSVG(file=paste("../Figures/NT5007_noisy.svg"), width = 4, height = 3, bg = "white")
#' merge_plate_layout_fits %>% dplyr::group_by(Bug_ID,Plate_no,compound,Replicate_no) %>% 
#'   filter(Bug_ID == 'NT24007' & compound %in% c('DMSO') & Plate_no %in% c('plate1')) %>%
#'   ggplot(aes(x=time, y=fitted, group=color)) + geom_line(aes(color=Replicate_no), size=0.4) + scale_color_uchicago() +
#'   geom_point(aes(y=OD, color=Replicate_no), size=0.5) + facet_wrap("compound", scales = "free") +  
...
#' CairoSVG(file=paste("../Figures/NT5007_noisy.svg"), width = 4, height = 3, bg = "white")
#' merge_plate_layout_fits %>% dplyr::group_by(Bug_ID,Plate_no,compound,Replicate_no) %>% 
#'   filter(Bug_ID == 'NT5037' & compound %in% c('DMSO') & Plate_no %in% c('plate1')) %>%
#'   ggplot(aes(x=time, y=fitted, group=color)) + geom_line(aes(color=Replicate_no), size=0.4) + scale_color_uchicago() +
#'   geom_point(aes(y=OD, color=Replicate_no), size=0.5) + facet_wrap("compound", scales = "free") +  
#'   scale_x_continuous(name = "Time (Hours)", limits = c(0,25)) + scale_y_continuous(name="bg corrected OD") + 
#'   th + theme_minimal()
...
# NT5076 not growing in any replicate (remove from analysis)
...
df1<-inner_join(fit_params_layout,noisy_wells_handpicked, by=c("Bug_ID","Plate_no","col_name","tech","Replicate_no"))
#View(df1)
nrow(df1) #533

params_bugs_cmp_filtered<-dplyr::anti_join(fit_params_layout,df1)
...
questionable_fits<-params %>% filter(note != '')
#View(questionable_fits)
write.table(questionable_fits, file = '../questionable_fits', sep = '\t', row.names = FALSE)
...
PC<-params %>% filter(compound == 'DMSO') %>% dplyr::group_by(Bug_ID,Plate_no,Replicate_no,Phyla,Sp_short) %>%
  mutate(ctrl_med = round(median(auc_l),digits = 2))
...
nrow(log2FC %>% filter(pv < 0.05)) #334
...
#filter_hits1<-hits %>% filter(pv_bh < 0.05 & (l2FC >= 0.2 | l2FC <= -0.2)) #33
#View(filter_hits1)
...
save(filter_hits, file = "filter_hits.rda")

load("filter_hits.rda")
...
############################# combination compound alone effects
head(cnormAUCs)
...
write.table(c_p_bh, file = "../Figures/cnormAUCs_combAlone", sep = '\t', row.names = FALSE)

c_p_bh %>% filter(pv_bh < 0.05) %>% dplyr::group_by(Bug_ID,Replicate_no,Plate_no) %>% ggplot(aes(x=compound, y=log2(avFC))) + geom_boxplot(aes(fill=compound), lwd=0.1, alpha=0.8) +
...
A<-read.table(file = "../Figures/Sig_bliss_scores", sep = "\t", header = TRUE)
...
save(sig_bliss, file = "sig_bliss.rda")
...
sig_bliss %>% filter(bliss_null > 0.2 | bliss_null < -0.2 & !(compound %in% c('MIN-3-035','MIN-0-60','MIN-3-100'))) %>% ggplot(aes(x=Sp_short,y=compound)) + facet_grid(~Plate_no, scales = 'free') + geom_tile(aes(fill=bliss_null)) + theme_bw() +
  th + theme(axis.text.x = element_text(angle = 90,hjust = 1), axis.ticks = element_blank(), panel.grid = element_blank()) + 
  scale_fill_viridis_b() + scale_y_discrete(name = "compounds") 
dev.off()
